...
pub struct ReplaceIdentifierWithFixedValueNotAssignedAtDeclaration;

impl Transform for ReplaceIdentifierWithFixedValueNotAssignedAtDeclaration {
...
impl<'a> Visitor<'a> {
    ...
    fn load_map_for_statement_list(&mut self, stmts: &[Statement<'a>]) {
        self.map = self.collect_candidates_not_assigned_at_declaration(stmts);
    }
}
